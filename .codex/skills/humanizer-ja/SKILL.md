...
技術書の執筆で `content-first-writing` と併用する場合は、ユーザーの回答を段落の一部に反映する時点で適用する。前書きでは作者の動機と情けない過去の具体性を残し、技術説明では読者の疑問への答えが薄まらないようにする。ここでいうユーザーは取材に答える相手、読者は完成稿を読む「君」である。
...
原則:
ですます調の中に、体言止めや短い断定文を混ぜる。全部同じ語尾にしない。
...
原則:
結論には自分の体温を入れる。「考えられます」ではなく「思ってます」「感じてます」。
...
「自分の場合は〜」「うちでは〜」「実際に試したら〜」
2. 意見を1つ入れる。
「正直、これは微妙だと思ってます」「個人的にはこっちの方がいい」
3. リズムを崩す。
長い文の後に短い文。丁寧語の後に体言止め。均一さを壊す。
4. 不完全さを認める。
「まだ試してない」「ここは自信ない」。完璧な文章はAIっぽい。
