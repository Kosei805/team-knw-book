...
この章では、部品の入手からCircuitPythonの書き込み、ライブラリの配置、エディタの準備までを済ませます。読み終えたときには、自分の書いたコードをPico Hで動かせる環境が手元にできています。
...
足の折れ曲がりや紛失で、予想以上に本数が必要になることがあるためです。
...
circuitpython.org のダウンロードページ（https://circuitpython.org/downloads）を開き、ボード一覧から「Raspberry Pi Pico」を選びます。
...
ライブラリは、 circuitpython.org のライブラリ配布ページ（https://circuitpython.org/libraries）からまとめて入手します。
...
ライブラリまで揃えても、コードを書いて Pico H に送る道具がまだありません。
