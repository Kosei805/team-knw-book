...
この章では、最初から完成版のプログラムを読むことはしません。各段階で「いま手元にあるコードに足りないものは何か」を考え、その不足を埋めるコードを追加します。最後に、それぞれの段階で作ったコードをつなぎ、実行ファイルとして保存します。

作るものは、Raspberry Pi Pico H、ブレッドボード、タクトスイッチ1個で構成したキーボードです。Raspberry Pi Pico Hは、プログラムを書き込んで回路を動かす小さなコンピューター基板です。ブレッドボードは、部品やワイヤーを穴へ差し込んで、はんだ付けなしで回路を組むための板です。タクトスイッチは、指で押している間だけ内部の電気の通り道がつながる部品です。Pico Hとスイッチをワイヤーでつなぎ、押すたびにパソコンへAキーを1回送ります。

この章を読み終えると、スイッチを押したという物理的な出来事が、電圧の変化、押下イベント、キーコード、HIDレポートという順で変換されてキー入力になる流れを、回路とコードの両方から追えるようになります。手元では、押すたびにAが1文字入力されるキーボードが動いている状態がゴールです。

== まず回路を作る

=== 回路の目的を決める

今回の回路で知りたいのは、スイッチが押されているかどうかだけです。Pico Hには、電圧を読み取る端子がいくつもあります。端子は、基板から出ている金属部分で、ワイヤーをつなぐ場所です。GPIO（General Purpose Input/Output）は、プログラムから電圧を読み取ったり、電圧を出力したりできる端子です。そのうち今回は、GPIO 0を表す@<tt>{GP0}を使います。

もう一つ使う@<tt>{GND}は、回路の電圧を測る基準になる端子です。ここでは0V側として使います。GP0とGNDの間をスイッチでつなぐと、スイッチを押したときだけGP0の電圧がGND側へ変わります。GND側は、スイッチを押しても0Vのままです。

スイッチを押していないときのGP0はHigh、押したときのGP0はLowにします。押していないときのHighは、Pico Hの内部プルアップで作ります。プルアップは、スイッチを離して入力先がどこにもつながっていないときでも、入力をHigh側へ保つ設定です。この回路にすると、外付け抵抗を使わずに、スイッチを押したときだけGP0をGNDへ引き下げられます。
...
まず、Pico Hをブレッドボード中央の溝をまたぐように、横向きに置きます。USBコネクタが左を向く向きにしてください。ピンがぐらつかないよう、ブレッドボードの奥までしっかり差し込みます。

次に、タクトスイッチを中央の溝にまたがせます。4本の端子を、左側の@<tt>{e23}、@<tt>{e25}、右側の@<tt>{f23}、@<tt>{f25}へ入れてください。スイッチの端子が23行と25行に並び、溝の左右に2本ずつ分かれる向きです。タクトスイッチは、同じ側にある2本の端子が内部でつながっています。今回使うのは、スイッチの左右から1本ずつです。


Pico Hのシルク印刷（基板表面に印刷された端子名）で@<tt>{GP0}と@<tt>{GND}の位置を確認します。図の向き（USBコネクタが左）では、Pico H側の@<tt>{a1}が@<tt>{GP0}、@<tt>{a3}が@<tt>{GND}です。基板の向きを変えた場合は穴番号をそのまま使わず、必ず端子名を見てください。赤いジャンパーワイヤーは、Pico H側の@<tt>{a1}とスイッチ右側の@<tt>{h23}をつなぎます。@<tt>{h23}は、タクトスイッチの右側にある@<tt>{f23}と同じ接続列です。青いジャンパーワイヤーは、Pico H側の@<tt>{a3}とスイッチ左側の@<tt>{c25}をつなぎます。@<tt>{c25}は、タクトスイッチの左側にある@<tt>{e25}と同じ接続列です。

図と向きが違う場合は、Pico Hの裏面にある端子名も見て、@<tt>{GP0}と@<tt>{GND}の位置を確認してください。
...
テスターがなくても、ここまでの配線を目視で確認できれば先へ進めます。持っている場合は、電源を入れる前に導通を確認してください。スイッチを離しているときはGP0とGNDが導通せず、押しているときだけ導通すれば、今回必要な回路になっています。

== GP0の電圧を読む

GP0のようなピンは、入力にも出力にも使える端子です。どちらとして使うかは、プログラムで設定します。回路ができただけでは、GP0の使い方はまだ決まっていません。

入力と出力の違いは、ピンの電圧をどちら側が決めるかです。入力は、外の回路が決めた電圧をプログラムで読み取る使い方です。出力は、プログラムが決めた電圧を外の回路へ出す使い方です。今回の回路で電圧を決めるのはスイッチで、Pico H側はその結果を読むだけです。そこで、GP0を入力にします。
...
この段階で必要なのは、GP0の電圧をプログラムから読むことです。次のコードは入力確認用のプログラムです。
...
この段階では、次のコードをそのまま@<tt>{code.py}として保存して動かします。ThonnyのShellに表示された値を見ながら、スイッチを押してください。@<tt>{time.sleep()}は表示を読みやすくするために使います。キーボード入力を作る段階では、この待ち時間は使いません。
...
生値が変わったら、確定を保留します。その後、20ミリ秒以上同じ値が続いたら確定値を更新します。確定値がHighからLowへ変わった場合にだけ、押下イベントを作ります。このように、入力値が落ち着くまで判定を待つ処理をデバウンスと呼びます。デバウンスは、チャタリングを1回の押下として扱うための処理です。
...
@<tt>{_fell}は、確定した押下があることを一時的に記録する内部フラグです。これをループから直接読む設計にすると、読んだあとに自分で@<tt>{False}へ戻す責任が、デバウンスの利用側へ漏れます。そこで、押下イベントを読む処理を1か所にまとめます。
...
@<tt>{fell}を1回読むと、TrueだったフラグがFalseへ戻ります。つまり、@<tt>{fell}は「押下イベントがあるか」を確認し、そのイベントを受け取ると同時に消費する窓口です。同じイベントを次のループで再処理しないために、この形にしています。イベントをデバッグ表示するときも、@<tt>{print(switch.fell)}と条件判定の両方で読まないようにします。
...
呼び出し側が必要とするのは、接点の揺れを含む生値ではなく、デバウンスを通過した@<tt>{fell}です。メインループでは毎周回@<tt>{update()}を呼んで内部状態を進め、押下イベントを使いたい場所でだけ@<tt>{fell}を1回読みます。@<tt>{update()}を呼ぶことと、イベントを読むことは別の役割です。

== Aキーを表す番号にする

ここまでで、プログラムは「押された」というイベントを持てるようになりました。しかし、イベントの値は真偽値であり、USBキーボードが必要とするAキーの情報ではありません。

文字列の@<tt>{"A"}を送ればよいように見えますが、キーボードは文字列を送る装置ではありません。たとえばAキーだけを押すと小文字の「a」になり、ShiftとAキーを同時に押すと大文字の「A」になります。同じAキーでも、組み合わせるキーによって結果が変わるためです。キーボードが報告するのは、どのキーと修飾キーが押されたかです。
...
Aだけを押しているときのレポートは、@<tt>{[0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00]}です。先頭は修飾キー、2番目は予約領域、その後ろに通常キーを並べます。空いている場所は0で埋めます。ShiftとAを同時に押す場合は、先頭の修飾キーの欄にShiftのビットを立て、Aのキーコードも残します。左Shiftなら、レポートは@<tt>{[0x02, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00]}になります。

@<tt>{send_report()}は、配列をUSB HIDデバイスへ渡します。ここでも文字を作っていません。OSがこのレポートを前回のレポートと比較し、Aが押された状態になったと解釈します。

=== 押下と解放を一つの操作にする

押下レポートを送ったあとに、キーを離した空のレポートを送り忘れると、OSからはキーが押されたままに見えます。この解放漏れを避けるため、レポートを2回送る処理を呼び出し側へ毎回書くのではなく、押下状態を送る@<tt>{press()}と、すべて空にした状態を送る@<tt>{release_all()}を、1回の入力としてまとめます。
...
@<tt>{send(Keycode.A)}は、Aを含むレポートを送ったあと、すべて0のレポートを送ります。これでOSから見た状態は「Aが押された」「Aが離された」になります。長押しや複数キーの保持を作る場合は、@<tt>{send()}ではなく@<tt>{press()}と@<tt>{release_all()}を組み合わせます。
...
上の断片で省略したimportも含め、実際にPico Hへコピーするファイルは次の内容です。
