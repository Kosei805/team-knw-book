...
次に、タクトスイッチを中央の溝にまたがせます。4本の端子を、左側の@<tt>{e25}、@<tt>{e26}、右側の@<tt>{f25}、@<tt>{f26}へ入れてください。スイッチの端子が25行と26行に並び、溝の左右に2本ずつ分かれる向きです。タクトスイッチは、同じ側にある2本の端子が内部でつながっています。したがって、@<tt>{e25}と@<tt>{e26}を別々の信号線だと思わないでください。今回使うのは、スイッチの左右から1本ずつです。

4端子のタクトスイッチは、向かい合う2端子がそれぞれ内部でつながっています。片側の2端子を別々の信号線だと思ってはいけません。GP0へつなぐ端子とGNDへつなぐ端子を、スイッチの反対側の組から1本ずつ選びます。

Pico Hのシルク印刷（基板表面に印刷された端子名）で@<tt>{GP0}と@<tt>{GND}の位置を確認します。図の向き（USBコネクタが左）では、Pico H側の@<tt>{a1}が@<tt>{GP0}、@<tt>{a3}が@<tt>{GND}です。基板の向きを変えた場合は穴番号をそのまま使わず、必ず端子名を見てください。赤いジャンパーワイヤーは、Pico H側の@<tt>{a1}とスイッチ右側の@<tt>{h25}をつなぎます。@<tt>{h25}は、タクトスイッチの右側にある@<tt>{f25}と同じ接続列です。青いジャンパーワイヤーは、Pico H側の@<tt>{a3}とスイッチ左側の@<tt>{c26}をつなぎます。@<tt>{c26}は、タクトスイッチの左側にある@<tt>{e26}と同じ接続列です。
